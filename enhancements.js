  ...
  if (typeof Swiper !== 'undefined') {
    ...
    if (tSwiper) {
      new Swiper('.testimonials-swiper', {
        slidesPerView: 1,
        spaceBetween: 24,
        loop: false, // disabled loop to avoid Swiper warning when slides < slidesPerView*2
...
/* ── BEFORE / AFTER SLIDERS (card version) ───────────────────────────────── */
document.querySelectorAll('.ba-card').forEach(card => {
  ...
  function updateSlider(val) {
    ...
    if (beforeLabel) beforeLabel.style.display = val > 60 ? 'none' : 'block';
    if (afterLabel)  afterLabel.style.display  = val < 40 ? 'none' : 'block';
  }
  updateSlider(50);
...
document.querySelectorAll('.hero-btn-primary, .fcta-btn-primary, .nav-cta-btn').forEach(btn => {
  btn.addEventListener('mousemove', (e) => {
    ...
    btn.style.transform = `translateY(-3px) translate(${x * 0.2}px, ${y * 0.2}px)`;
  });
...
function initPopup() {
  ...
  try {
    if (sessionStorage.getItem('popupShown')) { popupShown = true; return; }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const closePopup = () => {
    ...
    setTimeout(() => { popupOverlay.classList.remove('closing'); }, 400);
  };
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  searchForms.forEach(form => {
    ...
    if (input) {
      input.addEventListener('focus', () => { form.style.transform = 'scale(1.02)'; });
      input.addEventListener('blur',  () => { form.style.transform = 'scale(1)'; });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  categoryLinks.forEach(link => {
    ...
    link.addEventListener('mouseenter', () => { link.style.marginLeft = '8px'; });
    link.addEventListener('mouseleave', () => { link.style.marginLeft = '0'; });
  });
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (authorImg) {
    authorImg.addEventListener('mouseenter', () => { authorImg.style.boxShadow = '0 8px 25px rgba(202, 174, 95, 0.4)'; });
    authorImg.addEventListener('mouseleave', () => { authorImg.style.boxShadow = '0 4px 15px rgba(202, 174, 95, 0.3)'; });
  }
});
